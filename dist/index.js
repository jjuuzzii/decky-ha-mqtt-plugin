...
const reportStreaming = callable("set_streaming");
function Content() {
    ...
    const [streaming, setStreaming] = SP_REACT.useState(false);
    SP_REACT.useEffect(() => {
        ...
        const streamingListener = addEventListener("streaming_changed", (active) => {
            setStreaming(active);
        });
...
var index = definePlugin(() => {
    ...
    let devicesHook;
    try {
        devicesHook = SteamClient?.RemotePlay?.RegisterForDevicesChanges?.((devices) => {
            try {
                const anyStreaming = Array.isArray(devices) && devices.some((d) => d?.status === "Streaming");
                reportStreaming(anyStreaming).catch((e) => console.error("mqtt-status: reportStreaming failed", e));
            }
            catch (e) {
